...
void network_connection(int sockfd)
{
  ...
  if (election != 1){
    ...
    struct connect * customed_connect=(struct connect *)malloc(sizeof(struct connect));
    customed_connect->header.basic_header.byte=B_CONNECT;
    printf("Tienes/Deseas credenciales para realizar el CONNECT:\n 1.Sí\n 2.No\n");
    scanf_r = scanf("%d", &election);
    if(election !=1){
      printf("Ingresa un ID de cliente válido maximo 23 characteres alfanumericos:\n");
